...
class Game (var dimension : Int, private var debug : Boolean) {
    ...
    fun is_finish(): Boolean{
        return (this.gameState == GameState.WonHuman || this.gameState == GameState.WonAI)
    }

    fun return_winner(): Player{
        var result = Player.Human

        if (this.gameState == GameState.WonAI){
            result = Player.AI
        }
    ...
    fun unsunk_ships(): ArrayList<ShipType>{
        var result = ArrayList<ShipType>()
        boards[Player.Human]!!.placedShips.forEach { ship ->
            if (!ship.isSunk()) {
                result.add(ship.shipType)
            }
        }
        return result
    }
